(ns storm-topology.core
    (:require [backtype.storm clojure config :refer :all])
...
(defn run-local! [topology]
      (let [cluster (LocalCluster.)]
           (.submitTopology cluster "word-count" {TOPOLOGY-DEBUG true} topology)
           (Thread/sleep 10000)
           (.shutdown cluster)))

(defn submit-topology! [name topology]
      (let [options {TOPOLOGY-DEBUG true TOPOLOGY-WORKERS 3}]
